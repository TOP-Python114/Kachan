/*1. Вывести номера корпусов, если суммарный фонд фи-
нансирования расположенных в них кафедр превы-
шает 100000.*/
...
d.`building`
	...
	join(select 
				...
				sum(f.`financing`) as count_financing
					...
					group by
						f.`name`) a 
                        on a.`building` =d.`building`  and `count_financing`>100000
;

/*2. Вывести названия групп 5-го курса кафедры “Software
Development”, которые имеют более 10 пар в первую
неделю.*/
...
`avg_raiting`
	from `rating`
where `avg_raiting` > (select `avg_raiting`
	from `rating` where `name`= 'D221' )
;
...
`surname`
	...
	where `salary` > (select avg(`salary`) 
		from `teachers_salary` where `isprofessor` =  1 )
  ;  
...
рейтинг всех студентов группы) меньше, чем мини-
мальный рейтинг групп 5-го курса.*/
...
`avg_raiting`
	from `rating`
where `avg_raiting` < (select min(`avg_raiting`)
	from `rating` where `year`= 5 )
;

/*7. Вывести названия факультетов, суммарный фонд фи-
нансирования кафедр которых больше суммарного
...
`name_faculties`
	from `sum_financing`
where `count_financing` > (select `count_financing`
	from `sum_financing` where `name_faculties` = 'Computer Science')
;
/*8. Вывести названия дисциплин и полные имена препо-
давателей, читающих наибольшее количество лекций
по ним.*/
with data as (
select
...
на кафедре “Software Development”.*/
use academy;	
select 
