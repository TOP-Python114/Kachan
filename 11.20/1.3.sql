/*3.Создайте базу данных «Музыкальная коллекция». База
данных должна содержать информацию о музыкальных
дисках, исполнителях, стилях. Необходимо хранить сле-
дующую информацию:
...
■■ о песнях: название песни, название диска, длитель-
ность песни, музыкальный стиль песни, исполнитель.*/
...
alter table `Disks` 
	add constraint `FK_Disks_ArtistId` foreign key(`ArtistId`) 
								  references `Artists`(`Id`),
	add constraint `FK_Disks_PublisherId` foreign key(`PublisherId`) 
								references `Publishers`(`Id`),
	add constraint `FK_Disks_GenreId` foreign key(`GenreId`) 
								references `Genres`(`Id`);

alter table `Songs` 
	add constraint `FK_Songs_ArtistId` foreign key(`ArtistId`) 
								  references `Artists`(`Id`),
	add constraint `FK_Songs_DiskId` foreign key(`DiskId`) 
								references `Disks`(`Id`),
