...
class TestCase:
    def __init__(self):
        ...
        # КОММЕНТАРИЙ: у вас не столько история команд получилась, сколько история результатов — это тоже хорошо, но другое — по этому списку совсем непонятно, какая именно команда выполнялась
        # УДАЛИТЬ: к тому же, этот класс является адресатом команд — его код вам может вообще недоступен быть, чтобы так редактировать
        self.list = []
        self.msg: str = ''
        self.nums: int = 0
...
# КОММЕНТАРИЙ: я рад, что идеи абстракции вам пришлись по вкусу =)
...
# ДОБАВИТЬ: а этот класс является инициатором команд — его вы как раз и пишете — поэтому здесь имеет смысл хранить историю команд
class AllCommands:
    """
...
# СДЕЛАТЬ: перепишите код таким образом, чтобы в нём была история именно команд


# ИТОГ: доработать и сообщить об этом — 7/12
